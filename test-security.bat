...
for /f "tokens=*" %%i in ('curl -s -X POST %BASE_URL%/auth/login -H "Content-Type: application/json" -d "{\"usernameOrEmail\":\"bogdan\",\"password\":\"password123\"}" ^| jq -r .accessToken') do set ACCESS_TOKEN=%%i

if "%ACCESS_TOKEN%"=="null" (
    echo Login failed! Using default user...
    for /f "tokens=*" %%i in ('curl -s -X POST %BASE_URL%/auth/login -H "Content-Type: application/json" -d "{\"usernameOrEmail\":\"admin\",\"password\":\"password123\"}" ^| jq -r .accessToken') do set ACCESS_TOKEN=%%i
)
